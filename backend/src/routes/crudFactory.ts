...
/** Builds a permission-gated, soft-delete CRUD router scoped to the caller's
 *  active organization. Every query is filtered by organization_id so one
 *  organization can never read or mutate another's data. */
export function crudRouter(cfg: CrudConfig): Router {
  const router = Router();
  router.use(requireAuth, loadContext, requireOrg);
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const clauses = [`${alias}.organization_id = $1`, `${alias}.deleted_at is null`];
    ...
    asyncHandler(async (req, res) => {
      const { rows } = await query(
        `select ${alias}.*${cfg.selectExtra ? ', ' + cfg.selectExtra : ''}
         from ${T} ${alias} ${cfg.joins ?? ''}
         where ${alias}.id = $1 and ${alias}.organization_id = $2 and ${alias}.deleted_at is null`,
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const allCols = [...cols, 'organization_id', 'created_by'];
      const placeholders = allCols.map((_, i) => `$${i + 1}`);
      const { rows } = await query(
        `insert into ${T} (${allCols.join(', ')}) values (${placeholders.join(', ')}) returning *`,
        [...values, req.ctx!.orgId, req.user!.id],
      );
      await logActivity({ organizationId: req.ctx!.orgId, userId: req.user!.id, action: 'created', entity: cfg.table, entityId: rows[0].id, description: `Created ${cfg.module.slice(0, -1)}` });
  ...
  router.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      const { rows } = await query(
        `update ${T} set ${sets.join(', ')} where id = $1 and organization_id = $2 and deleted_at is null returning *`,
        [req.params.id, req.ctx!.orgId, ...cols.map((c) => body[c])],
      );
      if (!rows[0]) throw new ApiError(404, 'Not found');
      await logActivity({ organizationId: req.ctx!.orgId, userId: req.user!.id, action: 'updated', entity: cfg.table, entityId: req.params.id, description: `Updated ${cfg.module.slice(0, -1)}` });
      res.json({ data: rows[0] });
    }),
  );
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      const { rows } = await query(
        `update ${T} set deleted_at = now() where id = $1 and organization_id = $2 and deleted_at is null returning id`,
        [req.params.id, req.ctx!.orgId],
      );
      if (!rows[0]) throw new ApiError(404, 'Not found');
      await logActivity({ organizationId: req.ctx!.orgId, userId: req.user!.id, action: 'deleted', entity: cfg.table, entityId: req.params.id, description: `Deleted ${cfg.module.slice(0, -1)}` });
