...
reportsRouter.use(requireAuth, loadUser);
...
function gate(type: string) {
  return (req: import('express').Request, _res: import('express').Response, next: import('express').NextFunction) => {
    if (!canViewReport(req.appUser!.role, type)) return next(new ApiError(403, `Your role cannot view the ${type} report`));
    next();
  };
}

reportsRouter.get('/products', gate('products'), asyncHandler(async (_req, res) => {
...
reportsRouter.get('/customers', gate('customers'), asyncHandler(async (_req, res) => {
  const { rows } = await query(
    `select c.id, c.name, c.phone, c.outstanding_due, count(sa.id)::int as total_orders,
     ...
     where c.deleted_at is null group by c.id order by total_spent desc`);
  res.json({ data: rows });
}));

reportsRouter.get('/suppliers', gate('suppliers'), asyncHandler(async (_req, res) => {
  const { rows } = await query(
    `select s.id, s.name, s.phone, count(p.id)::int as total_orders,
     ...
     where s.deleted_at is null group by s.id order by total_purchased desc`);
  res.json({ data: rows });
}));
...
reportsRouter.get('/purchases', gate('purchases'), asyncHandler(async (req, res) => {
  ...
  const { rows } = await query(
    `select p.id, p.reference, p.purchase_date, p.total, s.name as supplier_name
     ...
     where p.deleted_at is null and p.purchase_date between $1 and $2 order by p.purchase_date desc`, [from, to]);
  res.json({ data: rows });
}));
...
reportsRouter.get('/sales', gate('sales'), asyncHandler(async (req, res) => {
  ...
  const { rows } = await query(
    `select sa.id, sa.invoice_no, sa.sale_date, sa.total, sa.paid, c.name as customer_name
     ...
     where sa.deleted_at is null and sa.sale_date between $1 and $2 order by sa.sale_date desc`, [from, to]);
  res.json({ data: rows });
}));
