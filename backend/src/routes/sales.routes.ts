...
salesRouter.use(requireAuth, loadUser);
...
salesRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    let where = 'sa.deleted_at is null';
  ...
  asyncHandler(async (req, res) => {
    const { rows } = await query(
      `select sa.*, c.name as customer_name, c.phone as customer_phone, c.address as customer_address, u.full_name as created_by_name
       ...
       where sa.id=$1 and sa.deleted_at is null`, [req.params.id]);
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await withTransaction(async (c) => {
      const sale = (await c.query(
        `insert into public.sales (customer_id, created_by, invoice_no, sale_date, subtotal, tax, discount, total, paid, payment_method, status, notes)
         values ($1,$2,$3,coalesce($4,current_date),$5,$6,$7,$8,$9,$10,'completed',$11) returning *`,
        [body.customer_id ?? null, req.user!.id, invoiceNo, body.sale_date ?? null, subtotal, body.tax ?? 0, body.discount ?? 0, total, body.paid ?? total, body.payment_method ?? 'cash', body.notes ?? null])).rows[0];
      for (const it of body.items) {
        const prod = (await c.query(`select id, name, stock from public.products where id=$1 and deleted_at is null for update`, [it.product_id])).rows[0];
        ...
        await c.query(`update public.products set stock = stock - $1 where id=$2`, [it.quantity, it.product_id]);
      ...
      if (body.customer_id && due > 0) await c.query(`update public.customers set outstanding_due = outstanding_due + $1 where id=$2`, [due, body.customer_id]);
      return sale;
    });
    await logActivity({ userId: req.user!.id, action: 'created', entity: 'sales', entityId: result.id, description: `Created invoice ${invoiceNo}` });
...
salesRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await withTransaction(async (c) => {
      const sale = (await c.query(`select * from public.sales where id=$1 and deleted_at is null for update`, [req.params.id])).rows[0];
      ...
      for (const it of old) await c.query(`update public.products set stock = stock + $1 where id=$2`, [it.quantity, it.product_id]);
      const oldDue = dueOf(Number(sale.total), Number(sale.paid));
      if (sale.customer_id && oldDue > 0) await c.query(`update public.customers set outstanding_due = greatest(0, outstanding_due - $1) where id=$2`, [oldDue, sale.customer_id]);
      ...
      if (body.customer_id && newDue > 0) await c.query(`update public.customers set outstanding_due = outstanding_due + $1 where id=$2`, [newDue, body.customer_id]);
    ...
    await logActivity({ userId: req.user!.id, action: 'updated', entity: 'sales', entityId: req.params.id });
...
salesRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    await withTransaction(async (c) => {
      const sale = (await c.query(`select id, customer_id, total, paid from public.sales where id=$1 and deleted_at is null for update`, [req.params.id])).rows[0];
      if (!sale) throw new ApiError(404, 'Not found');
      const items = (await c.query(`select product_id, quantity from public.sales_items where sale_id=$1`, [sale.id])).rows;
      for (const it of items) await c.query(`update public.products set stock = stock + $1 where id=$2`, [it.quantity, it.product_id]);
      const due = dueOf(Number(sale.total), Number(sale.paid));
      if (sale.customer_id && due > 0) await c.query(`update public.customers set outstanding_due = greatest(0, outstanding_due - $1) where id=$2`, [due, sale.customer_id]);
      await c.query(`update public.sales set deleted_at=now() where id=$1`, [sale.id]);
    });
    await logActivity({ userId: req.user!.id, action: 'deleted', entity: 'sales', entityId: req.params.id });
