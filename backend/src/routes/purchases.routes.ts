...
purchasesRouter.use(requireAuth, loadUser);
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await withTransaction(async (c) => {
      const purchase = (await c.query(
        `insert into public.purchases (supplier_id, created_by, reference, purchase_date, subtotal, tax, discount, total, notes)
         values ($1,$2,$3,coalesce($4,current_date),$5,$6,$7,$8,$9) returning *`,
        [body.supplier_id ?? null, req.user!.id, reference, body.purchase_date ?? null, subtotal, body.tax ?? 0, body.discount ?? 0, total, body.notes ?? null])).rows[0];
      for (const it of body.items) {
        const upd = await c.query(`update public.products set stock = stock + $1 where id=$2 and deleted_at is null returning id`, [it.quantity, it.product_id]);
    ...
    await logActivity({ userId: req.user!.id, action: 'created', entity: 'purchases', entityId: result.id, description: `Recorded purchase ${reference}` });
...
purchasesRouter.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { subtotal, total } = totals(body.items, body.tax ?? 0, body.discount ?? 0);
    const result = await withTransaction(async (c) => {
      const existing = (await c.query(`select id from public.purchases where id=$1 and deleted_at is null for update`, [req.params.id])).rows[0];
      ...
      for (const it of old) await c.query(`update public.products set stock = greatest(0, stock - $1) where id=$2`, [it.quantity, it.product_id]);
    ...
    await logActivity({ userId: req.user!.id, action: 'updated', entity: 'purchases', entityId: req.params.id });
...
purchasesRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    await withTransaction(async (c) => {
      const purchase = (await c.query(`select id from public.purchases where id=$1 and deleted_at is null`, [req.params.id])).rows[0];
      ...
      for (const it of items) await c.query(`update public.products set stock = greatest(0, stock - $1) where id=$2`, [it.quantity, it.product_id]);
      await c.query(`update public.purchases set deleted_at=now() where id=$1`, [purchase.id]);
    });
    await logActivity({ userId: req.user!.id, action: 'deleted', entity: 'purchases', entityId: req.params.id });
