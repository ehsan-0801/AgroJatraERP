...
accountsRouter.use(requireAuth, loadUser, requirePermission('accounts', 'read'));
  ...
  asyncHandler(async (req, res) => {
    ...
    const sales = (await query(
      `select count(*)::int as count,
       ...
       from public.sales where deleted_at is null and sale_date between $1 and $2`,
      [from, to],
    )).rows[0];
    ...
    const purchases = (await query(
      `select count(*)::int as count,
              ...
              coalesce(sum(tax),0)::float      as tax,
              coalesce(sum(discount),0)::float as discount
       from public.purchases where deleted_at is null and purchase_date between $1 and $2`,
      [from, to],
    )).rows[0];
    ...
    const cogs = Number((await query(
      `select coalesce(sum(si.quantity * p.purchase_price),0)::float as cogs
       ...
       join public.sales s on s.id = si.sale_id and s.deleted_at is null and s.sale_date between $1 and $2
       join public.products p on p.id = si.product_id`,
      [from, to],
    )).rows[0].cogs);

    // Current receivables across all customers (not range-bound)
    const receivable = Number((await query(
      `select coalesce(sum(outstanding_due),0)::float as due from public.customers where deleted_at is null`,
    )).rows[0].due);
    ...
    const salesByMonth = (await query(
      `select to_char(sale_date,'YYYY-MM') as month, sum(total)::float as v from public.sales
       where deleted_at is null and sale_date >= date_trunc('month', current_date) - interval '11 months'
       group by 1`,
    )).rows;
    const purchasesByMonth = (await query(
      `select to_char(purchase_date,'YYYY-MM') as month, sum(total)::float as v from public.purchases
       where deleted_at is null and purchase_date >= date_trunc('month', current_date) - interval '11 months'
       group by 1`,
    )).rows;
    ...
    if (type !== 'purchase') {
      parts.push(
        `select 'sale' as type, s.id, s.invoice_no as reference, s.sale_date as date,
         ...
         where s.deleted_at is null and s.sale_date between $1 and $2`,
      );
    }
    if (type !== 'sale') {
      parts.push(
        `select 'purchase' as type, p.id, p.reference, p.purchase_date as date,
         ...
         where p.deleted_at is null and p.purchase_date between $1 and $2`,
      );
    }
